...
fn record_id(run_id: u128, seq: usize) -> i64 {
    ((run_id << 32) as u64).wrapping_add(seq as u64) as i64
}
    ...
    ReportError: Display + Send + Sync + 'static,
{
    ...
    report_progress(
        &report,
        format!("✅ Ingestion completed -- rows/sec: {inserted_rows_per_sec:.0}."),
    )
    .await?;

    let point_query = if config.benchmark_point_query {
        Some(benchmark_point_query(&pool, &report, inserted_rows_per_sec).await?)
    } else {
        None
    };
    let range_query = if config.benchmark_range_query {
        Some(benchmark_range_query(&pool, &report, inserted_rows_per_sec).await?)
    } else {
        None
    };
    ...
    report_progress(
        &report,
        format!("✅ Ingestion completed -- rows/sec: {inserted_rows_per_sec:.0}; ✅ Querying matched {} rows in {:?}. Closing the Database...", query.matched_rows, query.elapsed),
    )
        .await?;
    ...
    let (passive_sync_elapsed, hard_sync_elapsed) = if config.run_wal_maintenance {
        report_progress(
            &report,
            "SQLite queries completed. Passively synchronizing WAL...".to_string(),
        )
        .await?;
        ...
        report_progress(
            &report,
            format!(
                "✅ Passive WAL Sync Completed in {passive_elapsed:?}. Hard syncing WAL underway..."
            ),
        )
        .await?;
        let start = Instant::now();
        ...
        report_progress(
            &report,
            format!("✅ Hard Sync Completed in {hard_elapsed:?}. Closing the Database..."),
        )
        .await?;
    ...
    report_progress(
        &report,
        format!("🏁 Ingestion completed -- rows/sec: {inserted_rows_per_sec:.0}; 🏁 Querying matched {} rows in {:?}; Database closed in {close_elapsed:?}.", query.matched_rows, query.elapsed),
    )
        .await?;
...
pub async fn benchmark_range_query<Report, ReportError>(
    pool: &Sqlite,
    report: &Report,
    inserted_rows_per_sec: f64,
) -> Result<QueryResult>
    ...
    ReportError: Display + Send + Sync + 'static,
{
    ...
    report_progress(
        report,
        format!(
            "✅ SQLite indexed querying completed -- matched_rows={matched_rows}, elapsed: {elapsed:.3?}."
        ),
    )
    .await?;
