...
use super::{
    ...
    render::{
        layout::{ChartLayout, PositionedText, build_layout, format_brl_cent, x_of_index, y_of_price},
...
impl RenderEngine {
    pub fn new(theme: Theme) -> Self {
        Self { theme, metrics: FontMetricsCache::default() }
    }
    ...
    pub fn plan(&mut self, quotes: &[Quote], moves: &[ImportantMove]) -> Result<ChartPlan> {
        ...
        let points: Vec<(i32, i32)> = quotes
            .iter()
            .enumerate()
            .map(|(idx, q)| (x_of_index(idx, quotes.len(), layout.plot_left, layout.plot_width), y_of_price(q.usd_brl, layout.y_min_cent, layout.px_per_cent, layout.plot_bottom)))
            .collect();
        ...
        Ok(ChartPlan { layout, points, movement_layers })
    }
    ...
    pub fn encode_png(&self, rgb: Vec<u8>) -> Result<(Vec<u8>, PngStats)> {
        let mode = self
            ...
            .mode;
        let png = match mode {
            PngEncodeMode::Fast => self.encode_png_with_image(&rgb, CompressionType::Fast, FilterType::Adaptive)?,
            PngEncodeMode::Balanced => self.encode_png_with_image(&rgb, CompressionType::Default, FilterType::Adaptive)?,
            PngEncodeMode::Level(level) => {
                let level = level.clamp(1, 9);
                self.encode_png_with_image(&rgb, CompressionType::Level(level), FilterType::Adaptive)?
            }
            PngEncodeMode::Uncompressed => self.encode_png_with_image(&rgb, CompressionType::Uncompressed, FilterType::NoFilter)?,
            PngEncodeMode::OxipngRaw { preset } => self.encode_png_with_oxipng_raw(rgb, preset)?,
        };

        let png_bytes = png.len();
        Ok((png, PngStats { mode: mode.name(), png_bytes }))
    }

    fn encode_png_with_image(&self, rgb: &[u8], compression: CompressionType, filter: FilterType) -> Result<Vec<u8>> {
    ...
    fn build_movement_layer(&mut self, mv: &ImportantMove, points: &[(i32, i32)], layout: &ChartLayout) -> Result<MovementLayer> {
            ...
            .is_some()
        {
            self.theme
                .movement
                .label_bg_padding_x_px
        } else {
            0
        };
        let bg_pad_y = if self
            .theme
            .movement
            .label_bg
            .is_some()
        ...
        let gap = self
            .theme
            .layout
            .movement_label_canvas_gap_px;
        x = clamp_i32(x, layout.plot_left + bg_pad_x + gap, layout.plot_right - bbox.width - bg_pad_x - gap);
        y = clamp_i32(y, layout.plot_top + bg_pad_y + gap, layout.plot_bottom - bbox.height - bg_pad_y - gap);
        ...
        Ok(MovementLayer {
            ...
            line_color,
            label_color,
            label: PositionedText { text: label, x, y, bbox },
        })
    }

    fn draw_plan<DB: DrawingBackend>(&self, root: &DrawingArea<DB, Shift>, plan: &ChartPlan) -> Result<()> {
        ...
        root.fill(&p.canvas_bg)
            .map_err(pe)?;

        root.draw(&Rectangle::new(
            [(layout.plot_left, layout.plot_top), (layout.plot_right, layout.plot_bottom)],
            p.plot_bg
                .filled(),
        ))
        .map_err(pe)?;
        ...
        for tick in &layout.y_ticks {
            root.draw(&PathElement::new(vec![(layout.plot_left, tick.y), (layout.plot_right, tick.y)], ShapeStyle::from(&p.grid).stroke_width(axis.grid_stroke_width_px)))
                .map_err(pe)?;
            self.draw_text(
                root,
                &tick.label,
        ...
        root.draw(&PathElement::new(vec![(layout.plot_left, layout.plot_top), (layout.plot_left, layout.plot_bottom)], axis_style))
            .map_err(pe)?;
        root.draw(&PathElement::new(vec![(layout.plot_left, layout.plot_bottom), (layout.plot_right, layout.plot_bottom)], axis_style))
            .map_err(pe)?;

        for tick in &layout.x_ticks {
            root.draw(&PathElement::new(
                vec![(tick.x, layout.plot_bottom), (tick.x, layout.plot_bottom + axis.x_tick_length_px)],
        ...
        for layer in &plan.movement_layers {
            root.draw(&PathElement::new(
                ...
                ShapeStyle::from(&layer.line_color).stroke_width(
                    self.theme
                        .movement
                        .line_width_px,
                ),
    ...
    fn draw_text<DB: DrawingBackend>(&self, root: &DrawingArea<DB, Shift>, text: &PositionedText, font_px: u32, color: RGBColor, transform: FontTransform) -> Result<()> {
