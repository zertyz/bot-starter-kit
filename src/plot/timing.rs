...
impl Timings {
    ...
    pub fn print_with_wall(&self, wall: Duration) {
        ...
        for (name, duration) in &self.rows {
            log::info!("  {:<24} {:>9.3} ms", name, duration.as_secs_f64() * 1000.0);
        }
        let measured_sum: Duration = self
            ...
            .map(|(_, d)| *d)
            .sum();
        log::info!("  {:<24} {:>9.3} ms", "measured sum", measured_sum.as_secs_f64() * 1000.0);
        log::info!("  {:<24} {:>9.3} ms", "wall total", wall.as_secs_f64() * 1000.0);
    }
}
