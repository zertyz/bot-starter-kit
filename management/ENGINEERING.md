...
3. Semantic management analysis: deterministic commands must keep four dimensions separate in machine-readable signals: deterministic quality findings; lexical semantic prompts with status `active` or `acknowledged`; `work_coverage`; and lifecycle-aware `evidence_coverage` with status `not_applicable`, `pending`, `evidenced`, or `gap`. Lexical prompts are never conclusions. Exact single-emphasis around a matched broad term (`*broad-term*`) acknowledges that term for the whole requirement, removing its active prompt while keeping the acknowledgement visible. Only rows parsed from the single canonical `## Current Links` table supply evidence; each row must name the exact requirement governing its work item, while parent views may aggregate descendant rows. Unmapped Requirement Areas are coverage acknowledgements, not evidence rows. Evidence gaps take precedence over pending active work, which takes precedence over an evidenced completion. Deterministic structural sizing must never be presented as a delivery estimate without semantic analysis of scope, dependencies, and implementation context. A repo-local AI skill must use those signals together with the governing requirements, backlog, traceability, source code, tests, and relevant git history to perform qualitative plan, verification, review, technical-debt, and requirement-drift analysis. Semantic reports must distinguish observed evidence, inference, and unknowns. The human owner controls requirement changes and final acceptance.
